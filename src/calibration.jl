...
!!! note
    Both `calibrate_fix_variables` and `calibrate_constraints` must be implemented
    for any subtype of `WiNDCtable`. If not implemented, an error will be thrown.
"""
function calibrate(
        data::T;
        silent::Bool = false,
